...
export const createHeader = ({ user, onLogout, onLogin, onCreateAccount }) => {
  ...
  if (user) {
    ...
    account.appendChild(createButton({ line: true, size: 'h32', label: 'Log out', onClick: onLogout }));
  } else {
    account.appendChild(createButton({ line: true, size: 'h32', label: 'Log in', onClick: onLogin }));
    account.appendChild(
      createButton({
        line: true,
        size: 'h32',
